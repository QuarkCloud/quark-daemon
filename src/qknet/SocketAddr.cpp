...
SocketAddr::SocketAddr() :sockAddr_{0}
{
    clear();
}
SocketAddr::SocketAddr(const SocketAddr& addr):sockAddr_{0}
{
    assign(addr);
}
SocketAddr::SocketAddr(const std::string& host, uint16_t port)
{
    assign(host, port);
}
...
{
    ::memset(&sockAddr_, 0, sizeof(SockAddrHolder));
}
bool SocketAddr::assign(const SocketAddr& addr)
{
    ::memcpy(&sockAddr_, &addr.sockAddr_, sizeof(SockAddrHolder));
    return true;
}
bool SocketAddr::assign(const std::string& addr)
{
    std::string host;
    uint16_t port = 0;
    if (SocketAddr::parseAddr(addr, host, port) == false)
        return false;
    return assign(host, port);
}
bool SocketAddr::assign(const std::string& host, uint16_t port)
{
    clear();
    uint32_t addr = 0;
...
bool SocketAddr::assign(const struct sockaddr & addr)
{
    clear();
    ::memcpy(&sockAddr_.sockAddr, &addr, sizeof(struct sockaddr));
    return true;
}
bool SocketAddr::assign(const struct sockaddr_in & addr)
{
    clear();
...
uint32_t SocketAddr::getInetAddr() const
{
...
const struct sockaddr * SocketAddr::getSockAddr() const
{
    return &sockAddr_.sockAddr;
}
int SocketAddr::getSocklen() const
{
    return sizeof(sockAddr_);
}
bool SocketAddr::parseAddr(const std::string& addr, std::string& host, uint16_t& port)
{
    host.clear();
    port = 0;
...
bool GetPeerAddrInfo(const std::string& host, std::vector<SocketAddr>& addrs)
{
