...
bool Socket::init(int type)
{
    ...
    socket_ = s;
    type_ = type;
    LOGCRIT("Socket[%p] handle[%p] created" , this , socket_);
    return true;
}
...
bool Socket::listen(int backlog)
{
    if (::listen(socket_, backlog) == SOCKET_ERROR)
    {
        LOGERR("socket[%p] faild to listen , errCode[%d]",
            getOsHandle(), ::WSAGetLastError());
        return false;
    }
    return true;
}

bool Socket::nodelay(bool value)
{
    int optvalue = 0;
    if (value == true)
        optvalue = 1;
...
bool Socket::localAddr(Address& addr) const
{
...
bool Socket::remoteAddr(Address& addr) const
{
    struct sockaddr name;
    int socklen = sizeof(name);
    ::memset(&name, 0, sizeof(name));
...
const OsHandle Socket::getOsHandle() const
{
